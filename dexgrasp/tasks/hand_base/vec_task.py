...
class VecTaskPython(VecTask):
    ...
    def step(self, actions, id=0):
        ...
        self.task.step(actions_tensor, id)

        if self.task.obs_buf.shape[-1] == 2567:
            return self.task.obs_buf.to(self.rl_device), self.task.rew_buf.to(self.rl_device), self.task.reset_buf.to(self.rl_device), self.task.extras
        else:
    ...
    def reserve_step(self, actions, id=0):
        actions_tensor = torch.clamp(actions, -self.clip_actions, self.clip_actions)

        self.task.step(actions_tensor, id)

        return torch.clamp(self.task.obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device), torch.clamp(self.task.obs_buf2, -self.clip_obs, self.clip_obs).to(self.rl_device), self.task.rew_buf.to(self.rl_device), self.task.reset_buf.to(self.rl_device), self.task.extras

    def reset(self):
        actions = 0.00 * (1 - 2 * torch.rand([self.task.num_envs, self.task.num_actions], dtype=torch.float32, device=self.rl_device))

        # step the simulator  
        self.task.step(actions, id=-1)
        if self.task.obs_buf.shape[-1] == 2567:
            return self.task.obs_buf.to(self.rl_device)
        else:
    ...
    def reserve_reset(self):
        ...
        return torch.clamp(self.task.obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device), torch.clamp(self.task.obs_buf2, -self.clip_obs, self.clip_obs).to(self.rl_device)
    
    # set the state of envs using state tensors, for motion planning
